...
class PlayList {
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.size; i++) {
            final_string += tracks[i].toString();
            final_string += "\n";
        }
        //// replace the following statement with your code
        return final_string;
    ...
    private int minIndex(int start) {
        ...
        Track minTrack = this.tracks[start];
        for (int i = start + 1; i < this.size; i++) {
            if (this.tracks[i].getDuration() < minTrack.getDuration()) {
                minTrack = this.tracks[i];
            }
        }
        return this.indexOf(minTrack.getTitle());
    }
    ...
    public void sortedInPlace() {
        ...
        for (int i = 0; i < this.size; i++) {
            for (int j = i; j < this.size; j++) {
                temp = this.tracks[j];
                loc_min = this.minIndex(j);
                this.tracks[j] = this.tracks[loc_min];
                this.tracks[loc_min] = temp;
            }
        }
